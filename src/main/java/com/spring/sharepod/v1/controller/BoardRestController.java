...
public class BoardRestController {
    ...
    // 9번 게시판 작성
    //1. 유저에게 받아온 정보를 (BoardRequestDto.WriteBoard)를 통하여 컨트롤러를러에서 토큰과, RequestPart를 이용하여 비디오,이미지 파일을 받은 후
    //2. 토큰을 비교하고나서 (awsS3Service.boardUpload)에서 파일들과 정보들을 저장하고, (boardService.wirteBoard)에서는 작성자의 user를 찾은 후
    //3. board의 정보들을 db에 저장하고 저장한 board id 를 통하여 imgfiles와 amount를 db에 저장한다.
    //4. 그후 서비스에서 (BoardResponseDto.BoardWrite)를 통해 저장 결과 값을 유저에게 보내준다.
    @PostMapping("/board")
